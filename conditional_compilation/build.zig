...
pub fn build(b: *std.Build) void {
    ...
    const fibonacci = b.addModule("fibonacci", .{ .source_file = .{ .path = fibonacci_file } });

    // To pass on the use_loop option to the program, use addOptions()
    ...
    // Add the options to the exe as an importable module called `config`
    exe.addOptions("config", options);
    // Add the importable fibonnaci module
    exe.addModule("fibonacci", fibonacci);

    // This declares intent for the executable to be installed into the
    // standard location when the user invokes the "install" step (the default
    // step when running `zig build`).
    // Generates the actual executable file
